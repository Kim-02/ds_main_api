"""이동 방향 검증 VLM — export_final.movement_validator 원본과 동일.

첫 프레임과 마지막 프레임을 비교 이미지로 합성해 VLM에게 이동 방향·위험도를 JSON으로 반환받는다.
"""
import json
import os
...
def compact_track(track):
    ...
    if "first_distance" in relation and "last_distance" in relation:
        result["d"] = [
            relation["first_distance"],
            relation["last_distance"]
        ]
...
def build_validation_payload(summary):
    ...
    return {
        "win": [
            ...
            summary.get("end_time", 0)
        ],
        "first": make_detection_payload(first_frame),
        "last": make_detection_payload(last_frame),
        "tracks": [compact_track(track) for track in summary.get("person_tracks", [])[:3]],
...
def make_compare_image(first_path, last_path, output_path, max_height=360):
    ...
    height = min(first.shape[0], last.shape[0])

    if height > max_height:
        height = max_height

    first_width = int(first.shape[1] * height / first.shape[0])
    last_width = int(last.shape[1] * height / last.shape[0])

    first = cv2.resize(first, (first_width, height))
    last = cv2.resize(last, (last_width, height))
    ...
    cv2.putText(combined, "LAST", (first_width + 35, 45), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 255), 3)
...
def build_validation_prompt(payload, max_chars):
    ...
    return limit_text(prefix + limit_text(payload_text, allowed), max_chars)
...
def normalize_validation_result(data):
    ...
    movement_valid = data.get("movement_valid", False)

    if movement_valid is not True:
...
class MovementValidator:
    ...
    def validate(self, summary, request_number):
        ...
        if first_frame is not None:
            first_path = first_frame.get("image_path", "")

        if last_frame is not None:
            last_path = last_frame.get("image_path", "")
